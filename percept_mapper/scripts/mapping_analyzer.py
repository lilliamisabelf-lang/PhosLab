...
class PhospheneMappingAnalyzer:
    ...
    def visualize_results(self, results, output_file="analysis_plot.png"):
        """Thin wrapper around `scripts.plots.plot_electrode_map`. The
        heavy lifting (matplotlib import, axis setup, ellipse math) now
        lives in `scripts/plots/electrode_map.py` so headless analyzers
        and notebooks can call it without touching this class."""
        if results is None:
        ...
        return plot_electrode_map(
            ...
            pixels_per_degree=(self.pixels_per_degree_x, self.pixels_per_degree_y),
